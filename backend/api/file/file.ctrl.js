...
exports.uploadFile = (req, res) => {
	...
	try {
		...
		return res.status(200).json({ status : 200, message : '파일 업로드에 성공하였습니다.' });
	} catch(err) {
		colorConsole.red(err.message);
		return res.status(500).json({ status : 500, message : '파일 업로드에 실패하였습니다.' });
	}
}
...
exports.downloadFile = async (req, res) => {
	...
	try {
		...
		fs.readFile(file, (err, data) => {
			if (err) {
				colorConsole.red(err.message);
				return res.status(500).json({ status : 500, message : '파일 다운로드에 실패하였습니다.' });		
			}
