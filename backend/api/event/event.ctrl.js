...
exports.deleteEvent = async (req, res) => {
	colorConsole.green('[EVENT] 이벤트 삭제');
	const { event_id } = req.query;
	...
	try {
		await models.File.deleteFileByEventId(event_id);
		await models.Event.deleteEvent(event_id);

		return res.status(200).json({ status : 200, message : '이벤트 삭제에 성공하였습니다.' });
	} catch(err) {
		colorConsole.red(err.message);
		return res.status(500).json({ status: 500, message : '이벤트 삭제에 실패하였습니다.' });
	}
}